...
self.addEventListener("install", event => {
  ...
  event.waitUntil(
    caches.open(version).then(cache => {
      return cache.addAll([
        // If even a single resource is not available then this promise will reject and no resource will be cached
        "/offline.html",
        "/scripts/offline.js",
        "/styles/offline.css",
        "/images/offline.jpeg"
...
self.addEventListener("fetch", event => {
  event.respondWith(
    caches.match(event.request).then(res => {
      if (res) return res;
      if (!navigator.onLine) return caches.match(new Request("offline.html"));
